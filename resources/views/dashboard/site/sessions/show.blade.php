@extends('dashboard.core.app')
@section('title', __('titles.sessions'))
@section('content')
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    {{--                    <h1>@lang('dashboard.Structure')</h1>--}}
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>

    <!-- Main content -->
    <section class="content">
        <div class="container-fluid">
            <!-- Content Wrapper. Contains page content -->
            <!-- Content Header (Page header) -->
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>@lang('dashboard.sessions')</h1>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="{{route('/')}}">@lang('dashboard.Structure')</a></li>
                                <li class="breadcrumb-item active">@lang('dashboard.sessions')</li>
                            </ol>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
            </section>

            <!-- Main content -->
            <section class="content">
                <div class="container-fluid">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">@lang('dashboard.sessions')</h3>
                            </div>
                            <div class="card-body">
                                <div class="col-12">
                                    <p class="lead">{{$session->name}}</p>
                                    <div class="table-responsive">
                                        <table class="table">
                                            <tbody>
                                            <tr>
                                                <th style="width:50%">@lang('dashboard.Name') @lang('dashboard.ar'):</th>
                                                <td>{{$session->first_title_ar}}</td>
                                            </tr>
                                            <tr>
                                                <th style="width:50%">@lang('dashboard.Name') @lang('dashboard.en'):</th>
                                                <td>{{$session->first_title_en}}</td>
                                            </tr>
                                            <tr>
                                                <th style="width:50%">@lang('dashboard.title_ar'):</th>
                                                <td>{{$session->second_title_ar}}</td>
                                            </tr>
                                            <tr>
                                                <th style="width:50%">@lang('dashboard.title_en'):</th>
                                                <td>{{$session->second_title_en}}</td>
                                            </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                    <table class="table table-bordered">
                                        <thead>
                                        <tr>
                                            <th style="width: 10px">#</th>
                                            <th>@lang('dashboard.writer_name')</th>
                                            <th>@lang('dashboard.title_ar')</th>
                                            <th>@lang('dashboard.title_en')</th>
                                            <th>@lang('dashboard.job_title') @lang('dashboard.ar')</th>
                                            <th>@lang('dashboard.job_title') @lang('dashboard.en')</th>
                                            <th>@lang('dashboard.link')</th>
                                        </tr>
                                        </thead>
                                        <p class="h2">@lang('dashboard.scientific-papers')</p>
                                        <tbody>
                                        @forelse($session->children as $key => $paper)
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{$paper->writer_name}}</td>
                                                <td>{{$paper->second_title_ar}}</td>
                                                <td>{{$paper->second_title_en}}</td>
                                                <td>{{$paper->job_title_ar}}</td>
                                                <td>{{$paper->job_title_en}}</td>
                                                <td><a href="{{$paper->finalUrl}}" target="_blank">@lang('dashboard.view_link')</a></td>
                                            </tr>
                                        @empty
                                            @include('dashboard.core.includes.no-entries', ['columns' => 6])
                                        @endforelse
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                            <!-- /.card-body -->
                        </div>
                        <!-- /.card -->
                    </div>
                    <!-- /.col -->
                </div>
                <!-- /.container-fluid -->
            </section>
            <!-- /.content -->
        </div>
        <!-- /.container-fluid -->
    </section>
    <!-- /.content -->
@endsection

@section('js_addons')

@endsection

